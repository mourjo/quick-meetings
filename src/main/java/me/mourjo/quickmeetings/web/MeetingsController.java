...
public class MeetingsController {
    ...
    ResponseEntity<MeetingCreationResponse> createMeeting(
        ...
        var fromZdt = ZonedDateTime.ofStrict(fromLdt,
            ZoneId.of(request.timezone()).getRules().getOffset(fromLdt),
            ZoneId.of(request.timezone())
        );

        var to = request.duration().to();
        var toLdt = LocalDateTime.of(to.date(), to.time());
        var toZdt = ZonedDateTime.ofStrict(toLdt,
            ZoneId.of(request.timezone()).getRules().getOffset(toLdt),
            ZoneId.of(request.timezone()));

        if (!fromZdt.isBefore(toZdt)) {
            throw new StartAfterEndException(fromZdt, toZdt);
        }
